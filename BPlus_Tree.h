...
int node::Find_Position(T key)//二分法更快
{
	...
	for (int i = 0; i < keynum; i++){
		...
		if (key <= Element)return int(values - block->buffer);//返回已经造成的偏移量
		values += index.attr_size + 4;
	}
	return int(values - block->buffer);//说明比所有key都大
}
...
void node::Split_Node(T key, int blk_ofst, INT32 position)
{
    ...
    {
        block -> getBlock(index.indexname, BlockAddr,0);
    }
    ...
    if (IsRoot())//根结点被分裂
    {
		Block *left = new Block, *right = new Block;
        left -> getBlock(index.indexname,0,1);
        right -> getBlock(index.indexname,0,1);
        ...
        memcpy(right->buffer + 8, &LEFT_FLAG, 4);//extra,暂时没什么用，除了root
        //更新新的块的块头信息
        ...
        memcpy(left->buffer+INDEX_BLOCK_INFO+last_ptr,&(right->BlockAddr) , 4);
        //至此叶结点处理完成，需要向上插入一个标记key
        T right_first_key;
        getElement(&right_first_key,right->buffer+INDEX_BLOCK_INFO+4,size);
        //分裂完成后，要取出右侧结点的第一个key作为root中唯一的key
        memcpy(block->buffer + 4, &const_number[1], 4);//keynum变成1
        memcpy(block->buffer, &const_number[3], 4);//更新type,变成了内部根结点
        memcpy(block->buffer + INDEX_BLOCK_INFO, &(left->BlockAddr), 4);
        setElement(block->buffer + INDEX_BLOCK_INFO + 4, &right_first_key, index.attr_size);
        memcpy(block->buffer + INDEX_BLOCK_INFO + 4 + index.attr_size, &(right->BlockAddr), 4);
    ...
    else//如果不是根叶结点被分裂，说明是叶结点，需要向上插入这个key值
    {
        ...
        memcpy(block->buffer + last_ptr, &(right->BlockAddr), 4);
...
void node::Insert_Key_To_Node(int blk_ofst, INT32 ptr, T key)
{
    ...
    {
        INT32 First_Son;
        memcpy(&First_Son,block->buffer+INDEX_BLOCK_INFO,4);
        Block *temp=getBlock(index.indexname,First_Son);
        getElement(&key,temp->buffer+INDEX_BLOCK_INFO+4,index.attr_size);
